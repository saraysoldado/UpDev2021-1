%\documentclass[reprint,superscriptaddress,groupedaddress,amsmath,amssymb,aps,floatfix,
%onecolumn,
%pre,
%]{revtex4-1}
\documentclass[
...
% \subsection{{\em Balanced-Homeostatic} learning rule}

% The equations for the Balanced-Homeostatic learning rule are
% \begin{equation}
% \begin{aligned}
...
% \begin{equation}
% \begin{aligned}
% a_1 + (b_1-c_1)d & < (b_1^\prime - c_1^\prime)e \\
% a_2 + (b_2 - c_2)d & < (b_2^\prime - c_2^\prime) e
% \end{aligned}
% \label{eq.BA_stable_cond_summary}
% \end{equation}
...
% b_1 & = I_{\set}^2 \Theta_E \, \alpha_1 g_E^2 g_I \\
% c_1 & = E_{\set}^2 I_{\set} \, \alpha_1 \, g_E^2 \\
% b_1^\prime & = E_{\set} I_{\set} \Theta_I \, \alpha_1 \, g_E g_I^2 \\
% c_1^\prime & = E_{\set}^2 I_{\set} \, \alpha_3 \, g_I^2 \\
% a_2 & = 2 \Theta_E \Theta_I \, \alpha_1 \, g_E^2 g_I^2 \\
% b_2 & = 2I_{\set} \Theta_E \, \alpha_1 \, g_E^2 g_I \\
% c_2 & = E_{\set}^2 \, \alpha_1 \, g_E^2 \\
% b_2^\prime & = 2E_{\set} \Theta_I \, \alpha_1 \, g_E g_I^2 \\
% c_2^\prime & = E_{\set}^2 \, \alpha_3 \, g_I^2 \\
% d & = W_{\II\up} \, g_I + 1 \\
% e & = W_{\EE\up} \, g_E -1
% \end{aligned}
% \end{displaymath}

% \noindent Conditions Eq.\ \ref{eq.BA_stable_cond_summary} are satisfied for biologically backed parameter values. See the step-by-step derivation of the stability condition in Section \ref{sec.BA_details}.
...
%\subparagraph{Down state} There's a trivial fixed point (i.e.\ a steady-state solution $dX/dt=0$) at $E=I=0$ when the inputs to both subpopulations are subthreshold. This fixed point is stable: if the currents are below the ReLU threshold, any value $E>0$ and $I>0$ will have a negative velocity leading to an exponential decrease towards zero.  
...
% \subsubsection{Two-dimensional dynamics}

% The general program to assess stability of an isolated fixed point involves computing the eigenvalues of the Jacobian at the fixed point and looking at their real parts: a single eigenvalue with positive real part means the fixed point is unstable, while if all eigenvalues have negative real part the fixed point is stable. In case no eigenvalues have positive real part but at least one has zero real part, the linear stability analysis is inconclusive and one must go to higher orders by computing the center manifold.

% However, the learning rules we study have non-isolated fixed points: the 2D planes defined by e.g.\ Eq.\ \ref{eq.upstate_W} where two weights are free. In this case, two out of the four eigenvalues are expected to be zero, meaning there is no dynamics along the 2D plane and thus there is no need to compute the center manifold.

% In order to show that this is the case and build confidence in our results, here we perform a simple linear transformation of our variables and arrive without any further approximation at a system where two of the variables have exact zero dynamics. See SageMath code in the Supplementary Material: {\tt up states - CrossHomeostatic stability.ipynb}.


% As a first step we switch from the nondimensionalized weights $w_{\EE},w_{\EI},w_{\IE},w_{\II}$ to new variables $w_1,w_2,w_3,w_4$ that are zero at the fixed point:
% \begin{equation}
% \begin{aligned}
% w_1 & = w_{\EE} - w_{\EE\up} \\
% w_2 & = w_{\EI} - w_{\EI\up} \\
% w_3 & = w_{\IE} - w_{\IE\up} \\
% w_4 & = w_{\II} - w_{\II\up}
% \label{eq.CH_transf_1}
% \end{aligned}
% \end{equation}

% \noindent where $w_{\EE\up}$ and $w_{\IE\up}$ are set to arbitrary values and $w_{\EI\up}$ and $w_{\II\up}$ are defined by Eq.\ \ref{eq.CH_upstate_w} (all satisfying the stability conditions of the neural subsystem, Eqs.\ \ref{eq.detcond_orig} and \ref{eq.tracecond_orig}). In the new variables, the learning rule Eq.\ \ref{eq.CH_full_norm_qss} reads:
% \begin{equation}
% \begin{aligned}
% \frac{dw_1}{d\tau} & = f_1(w_1,w_2,w_3,w_4) \\
% \frac{dw_2}{d\tau} & = f_2(w_1,w_2,w_3,w_4) \\
% \frac{dw_3}{d\tau} & = f_3(w_1,w_2,w_3,w_4) \\
% \frac{dw_4}{d\tau} & = f_4(w_1,w_2,w_3,w_4)
% \end{aligned}
% \label{eq.CH_full_norm_qss_v2}
% \end{equation}

% \noindent where
% \begin{displaymath}
% \begin{aligned}
% f_1 = -i_{\qss}(& w_1 + w_{\EE\up},w_2 + w_{\EI\up}, \\
% & w_3 + w_{\IE\up},w_4 + w_{\II\up}) + 1 \\
% f_2 = \mbox{\ldots etc.} &
% \end{aligned}
% \end{displaymath}

% \noindent according to the transformation Eq.\ \ref{eq.CH_transf_1}.

% The second step is a textbook diagonalization of the system. The Jacobian matrix of Eq.\ \ref{eq.CH_full_norm_qss_v2} has the following eigenvalues:
% \begin{displaymath}
% (0,0,\lambda_3,\lambda_4)
% \end{displaymath}

% \noindent where $\lambda_{3,4} < 0$ are the eigenvalues shown in Eq.\ \ref{eq.CH_eigvals}. With this we compute the corresponding eigenvectors $v_1,v_2,v_3,v_4$ (the double eigenvalue 0 has a geometric multiplicity of 2 so there is no need to compute generalized eigenvectors and Jordan form) and arrange them in columns to form the diagonalizing matrix $\mathbf{T}$. The transformation of the variables $(w_1,w_2,w_3,w_4) \rightarrow (w,x,y,z)$ is such that, in matrix notation,
% \begin{equation}
% (w_1,w_2,w_3,w_4) = \mathbf{T}
% (w,x,y,z)
% \label{eq.CH_transf_2}
% \end{equation}

% \noindent The transformation of the whole vector field, in matrix notation with $\mathbf{F} = (f_1,f_2,f_3,f_4)$, is
% \begin{equation}
% \frac{d(w,x,y,z)}{d\tau} = \mathbf{T}^{-1}\mathbf{F}(\mathbf{T}(w,x,y,z))
% \end{equation}

% \noindent which unfolded is
% \begin{equation}
% \begin{aligned}
% \frac{dw}{d\tau} & = 0 \\
% \frac{dx}{d\tau} & = 0 \\
% \frac{dy}{d\tau} & = \lambda_3 y + g_3(w,x,y,z) \\
% \frac{dz}{d\tau} & = \lambda_4 z + g_4(w,x,y,z)
% \end{aligned}
% \label{eq.CH_full_norm_qss_v3}
% \end{equation}

% \noindent showing that two directions have dynamics that is identically zero. Note that this result didn't involve any further approximation other than the QSS approximation of the neural subsystem.
...
% \subsection{Synaptic dynamics: {\em Balanced-Homeostatic} learning rule}
% \label{sec.BA_details}

% (SageMath code in the Supplementary Material: {\tt up states - Balanced stability.ipynb})
...
% In continuous-time dynamics, the Balanced-Homeostatic learning rule reads:
% \begin{equation}
% \begin{aligned}
% \frac{dW_{\EE}}{dt} & = +\alpha_1 g_E E(E_{\set} - E) \\
% \frac{dW_{\EI}}{dt} & = \frac{1}{\tau_0} (f(W_{\EE}) - W_{\EI}) \\
% \frac{dW_{\IE}}{dt} & = +\alpha_3 g_I E(I_{\set} - I) \\
% \frac{dW_{\II}}{dt} & = \frac{1}{\tau_0} (g(W_{\IE}) - W_{\II})
% \end{aligned}
% \label{eq.BA_equation}
% \end{equation}

% \noindent where
% \begin{equation}
% \begin{aligned}
% f(W_{\EE}) & = \frac{(E_{\set} W_{\EE} - \Theta_E) g_E - E_{\set}}{I_{\set} g_E} \\
% g(W_{\IE}) & = \frac{(E_{\set} W_{\IE} - \Theta_I) g_I - I_{\set}}{I_{\set}g_I}
% \end{aligned}
% \label{eq.BA_equation_fg}
% \end{equation}

% \noindent are the Up state fixed point values of the weights (see below). The only fixed point compatible with the neural subsystem being suprathreshold is the Up state
% \begin{equation}
% \begin{aligned}
% E_{\up} & = E_{\set} \\
% I_{\up} & = I_{\set} \\
...
% W_{\II\up} & = g(W_{\IE\up})
% \end{aligned}
% \label{eq.BA_upstate_set}
% \end{equation}

% \noindent where $W_{\EE\up}$ and $W_{\IE\up}$ are free (not all values will lead to stable Up states, though).


% \subsubsection{Nondimensionalization}

% The full system (Eqs.\ \ref{eq.BA_equation} and \ref{eq.neural}) in nondimensionalized form reads:
% \begin{equation}
% \begin{aligned}
% \epsilon_E \frac{de}{d\tau} & = -e + R g e w_{\EE} - i w_{\EI} - \theta_E \\
% \epsilon_I \frac{di}{d\tau} & = -i + \frac{e w_{\IE}}{\alpha g} - i w_{\II} - \theta_I \\
% \frac{dw_{\EE}}{d\tau} & = -(e - 1)e \\
% \frac{dw_{\EI}}{d\tau} & = -w_{EI} + R g w_{\EE} - \theta_E - 1 \\
% \frac{dw_{\IE}}{d\tau} & = +(i-1)e \\
% \frac{dw_{\II}}{d\tau} & = -w_{\II} + \frac{Rw_{\IE}}{\alpha g} - \theta_I - 1
% \end{aligned}
% \label{eq.BA_full_norm}
% \end{equation}
...
% \begin{equation}
% \begin{aligned}
% e_{\qss} & = (\theta_I w_{\EI} - \theta_E(w_{\II} + 1))\alpha g/c \\
% i_{\qss} & = (\alpha g \theta_I (R g w_{\EE} - 1) - R\theta_E w_{\IE})/c
% \end{aligned}
% \label{eq.BA_upstate_qss}
% \end{equation}

% \noindent where
% \begin{displaymath}
% \begin{aligned}
% c = (w_{\II} + 1)\alpha g(R g w_{\EE} - 1) - R w_{\EI} w_{\IE}
% \end{aligned}
% \end{displaymath}
...
% \begin{equation}
% \begin{aligned}
% \frac{dw_{\EE}}{d\tau} & = -(e_{\qss} - 1)e_{\qss} \\
% \frac{dw_{\EI}}{d\tau} & = -w_{EI} + R g w_{\EE} - \theta_E - 1 \\
% \frac{dw_{\IE}}{d\tau} & = +(i_{\qss}-1)e_{\qss} \\
% \frac{dw_{\II}}{d\tau} & = -w_{\II} + \frac{Rw_{\IE}}{\alpha g} - \theta_I - 1
% \end{aligned}
% \label{eq.BA_full_norm_qss}
% \end{equation}

% \noindent where $e_{\qss}$ and $i_{\qss}$ are nonlinear functions of the weights as defined by Eq.\ \ref{eq.BA_upstate_qss}.
...
% i_{\qss} & = 1
% \end{aligned}
% \label{eq.BA_upstate_ei}
% \end{equation}

% \noindent and
% \begin{equation}
% \begin{aligned}
% w_{\EI\up} & = R g w_{\EE\up} - \theta_E - 1 \\
% w_{\II\up} & = \frac{Rw_{\IE\up}}{\alpha g} - \theta_I - 1
% \end{aligned}
% \label{eq.BA_upstate_w}
% \end{equation}

% \noindent which are the nondimensionalized versions of Eq.\ \ref{eq.BA_upstate_set}. Weights $w_{\EE\up}$ and $w_{\IE\up}$ are free, so the Up state is a 2D plane of non-isolated fixed points.



% \subsubsection{Stability condition}

% Now we show the conditions for which this learning rule is stable. The Jacobian matrix of Eq.\ \ref{eq.BA_full_norm_qss} evaluated at the Up state has two zero eigenvalues (as expected) and two nonzero eigenvalues. The nonzero eigenvalues have the form:
% \begin{equation}
% \lambda_{\pm} = \frac{A \pm \sqrt{A^2 + FC}}{C}
% \label{eq.BA_eigvals}
% \end{equation}

% \noindent where
% \begin{displaymath}
% C = 2(\alpha g^2 \theta_I w_{\EE\up} - \theta_E w_{\IE\up}  - \frac{\alpha g\theta_I}{R})
% \end{displaymath}

% \noindent and $A$ and $F$ are long expressions that can be found in the corresponding Jupyter notebook.

% To determine the sign of Eq.\ \ref{eq.BA_eigvals}, first note that $C$ must be positive because it is related to one of the stability conditions of the neural subsystem (Eq.\ \ref{eq.detcond_orig}, after substituting back to dimensionalized quantities). Next note that both eigenvalues have negative real part if $F<0$ and $A<0$:  
% if $F < 0$ then the argument of the square root is less than  $A^2$, and then the square root itself is either real and less than $|A|$, or imaginary. In any case, if in addition $A<0$ then both eigenvalues have negative real part. The stability conditions are then $F<0$ and $A<0$, which can be written in terms of the dimensionalized parameters and variables as
% \begin{equation}
% \begin{aligned}
% a_1 + (b_1-c_1)d & < (b_1^\prime - c_1^\prime)e \\
% a_2 + (b_2 - c_2)d & < (b_2^\prime - c_2^\prime) e
% \end{aligned}
% \label{eq.BA_stable_cond}
% \end{equation}
...
% It is hard to analytically determine whether the stability conditions Eq.\ \ref{eq.BA_stable_cond} are satisfied or not in the general case. However, here we show that for the biologically backed set of parameter values in Table \ref{tab.params} (with the addition of $\alpha_1=0.002$ and $\alpha_3=0.00002$) the conditions are satisfied and the learning rule is thus stable.

% In Figure \ref{fig.BA_stability} we plot the stability conditions of this rule, after switching $W_{\II} \rightarrow W_{\IE}$ via Eq.\ \ref{eq.upstate_W}. It is clear that the stability region of the neural subsystem lies well within the stability region of the learning rule, making the full system stable.

% \begin{figure}[ht]
% \centering
% \includegraphics[width=\linewidth]{BA_stability}
% \caption{Regions of stability, {\em Balanced Homeostatic} rule. The neural subsystem is stable in a region (upper right green triangle) where the Balanced-Homeostatic rule is also stable. Every label is on the side where the  corresponding  condition  holds (synaptic stable 1 and 2: Eq.\ \ref{eq.BA_stable_cond}; paradoxical: Eq.\ \ref{eq.paradoxical}; detcond stable: Eq.\ \ref{eq.detcond_orig}; trcond stable: Eq.\ \ref{eq.tracecond_orig}; positive $W_{\EI}$: Eq.\ \ref{eq.positive_WEI}; positive $W_{\II}$: Eq.\ \ref{eq.positive_WII}).}
% \label{fig.BA_stability}
% \end{figure}
...
% \begin{figure}[ht]
% \centering
% \includegraphics[width=\linewidth]{SS_stability}
% \caption{Regions of stability, {\em SynapticScaling} rule. The rule is stable in a region that doesn't overlap with the region where the neural subsystem is stable. Every label is on the side where the  corresponding  condition  holds (synaptic stable: Eq.\ \ref{eq.SS_stable_cond}; paradoxical: Eq.\ \ref{eq.paradoxical}; detcond stable: Eq.\ \ref{eq.detcond_orig}; trcond stable: Eq.\ \ref{eq.tracecond_orig}; positive $W_{\EI}$: Eq.\ \ref{eq.positive_WEI}; positive $W_{\II}$: Eq.\ \ref{eq.positive_WII}).}
% \label{fig.SS_stability}
% \end{figure}
